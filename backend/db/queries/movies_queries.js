...
const getAllMovies = (req, res, next) => {
  db.any(
    'SELECT movies.id, movies.title, movies.img_url, AVG(ratings.stars) FROM movies JOIN ratings ON movies.id = ratings.movie_id GROUP BY movies.id, movies.title, movies.img_url ORDER BY title'
  )
    .then(data => {
      res.status(200).json({
        status: 'Success',
